...
@possible_answers = File.read('possibilities.txt').split

# @possible_guesses are the words you can type to guess
@possible_guesses = File.read('guesses.txt').split
@excluded_letters = []
@maybe_letters = [[], [], [], [], []]
@included_letters = {}
@found_letters = %w[? ? ? ? ?]
...
def load_or_create_distribution
  filename = "./cache/#{create_regex_safe_string}.json"
  if(File.exist?(filename))
    file = File.read(filename)
    JSON.parse(file)
  else
    distribution = create_word_distribution
...
def contains_excluded?(word)
  word.each_char do |letter|
    return true if @excluded_letters.include? letter
  end
  false
end
...
def parse_answer(answer)
  ...
  answer.each_char do |letter|
    case letter
    when 'n'
      # Skip excluding a letter that is already included.
      next if @included_letters.key?(@current_guess[i])
      @excluded_letters << @current_guess[i]
    when 'm'
      if @included_letters.key?(@current_guess[i])
        @included_letters[@current_guess[i]] += 1
      else
  ...
  puts "Excluded: #{@excluded_letters}"
